...
function socialOpt(out_dir, files_ID, instance1, demandsVec, polyDeg, free_flow_time, fcoeffs, capacity)
    ...
    #for a = 1:numLinks
    #    for i = 1:polyDeg
    #        cost += free_flow_time[a] * fcoeffs[i] * linkFlow[a]^i / (capacity[a]^(i-1));
    #    end
    #end

#    @NLexpression(m, f, cost);
...
function adjustingODdemands(instance, day, cnt, out_dir, month_w, month, year, files_ID, ta_data, link_list,numNodes, gamma1, gamma2 )
    ...
    #user_sol_dict[day] = socialObj(tapSocialFlowVecDict_[day], free_flow_time, polyDeg, fcoeffs, capacity, numLinks);
