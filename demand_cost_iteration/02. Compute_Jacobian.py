...
for instance in time_instances['id']:
    Compute_jacobian(instance)


import networkx as nx

def jacobianSpiess(numNodes, numLinks, numODpairs, od_pairs, instance, link_list_js, link_length_list):
    
    link_label_dict = zload( out_dir + 'link_label_dict_network.pkz')
    link_label_dict_ = zload( out_dir + 'link_label_dict_network_.pkz')

    with open(out_dir + 'od_pair_label_dict_refined.json', 'r') as json_file:
        OD_pair_label_dict_refined = json.load(json_file)
